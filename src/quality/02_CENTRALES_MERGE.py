...
RUTA_BASE_EC = Path(r"C:\A_GS1_PROYECTOS\DELETE\93XX-20260206T200049Z-1-001\93XX")
RUTA_BASE_IS = Path(r"\\atlas\VP_INFRAESTRUCTURA\G_Plan_Gestion_Proyectos\RI200_A_Admon_Capacidad_P_R_Infr\2023\REPORTES_INF_CART_GEO\DICCIONARIO_DE_DATOS\40_CABLES_CORPORATIVOS\12_Cables_troncales_ conectados_en central")
RUTA_ANILLOS_CAMBIO = Path(r"C:\A_GS1_PROYECTOS\ANILLOS-CAMBIO.xlsx")
...
IS_COL_FIBRA = "FIBRA"
IS_COL_EQUIPO = "EC"
IS_COL_INTERFAZ = "PUERTO C"
IS_COL_PATCHEO = "PATCHEO"
IS_COL_ODF_CALLE = "ODF CALLE"
IS_COL_HILO = "HILO"
IS_COL_OP = "ODF P"
...
POSIBLES_ID_BAY = ["Id Bay", "ID Bay", "id bay", "Bay Id", "BAY ID"]
...
def extraer_equipo_central(archivo):
    try:
        ...
        print(f"Error en extraer_equipo_central({archivo}): {e}", file=sys.stderr)
...
def extraer_anillo_desde_origen(valor):
    """Extrae el nombre del anillo de una cadena como TO_DEM_H6_70930_G2_MEADALBOSC06(10.254.238.227) -> MEADALBOSC06"""
    if pd.isna(valor):
        return None
    ...
    # Buscar patrón ME seguido de letras mayúsculas y números
    patrones = [
        r'(ME[A-Z0-9]+)',
        r'([A-Z]{2,}[A-Z0-9]{3,})',
    ]
...
def cargar_datos_anillos(ruta_anillos):
    """Carga el archivo de anillos y devuelve un diccionario con la información"""
    try:
        if not ruta_anillos.exists():
            print(f"Archivo no encontrado: {ruta_anillos}", file=sys.stderr)
            return {}
        ...
        for col in columnas_requeridas:
            if col not in df_anillos.columns:
                print(f"Columna '{col}' no encontrada en la hoja {HOJA_ANILLOS}", file=sys.stderr)
                return {}
        ...
        print(f"Error al cargar archivo de anillos: {e}", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
        return {}

def procesar_cambio_anillo(fibra, anillo, anillos_dict):
    """Procesa el estado del anillo y devuelve el formato V| o F|"""
...
def leer_un_archivo_ec(archivo):
    try:
        ...
        print(f"Error en leer_un_archivo_ec({archivo}): {e}", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
        return pd.DataFrame()
...
def generar_hoja_ec(ruta_carpeta):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for idx, future in enumerate(as_completed(futures), 1):
            sys.stdout.write(f"\rProcesando EC: [{idx}/{total}] archivos")
            sys.stdout.flush()
            try:
                df = future.result()
                if not df.empty:
                ...
                print(f"\nError en hilo para {futures[future]}: {e}", file=sys.stderr)
                traceback.print_exc(file=sys.stderr)
...
def leer_un_archivo_is(archivo):
    ...
    try:
        ...
        if ext in ('.xlsx', '.xlsm'):
            ...
            if target_sheet is not None:
                ...
                for col_idx, val in enumerate(header_row):
                    ...
                    if 'equipo' in val_lower:
                        col_indices['equipo'] = col_idx
                    elif 'interfaz' in val_lower and 'puerto' in val_lower:
                        col_indices['interfaz'] = col_idx
                    elif 'patcheo' in val_lower:
                        col_indices['patcheo'] = col_idx
                    elif 'odf' in val_lower and 'calle' in val_lower:
                        col_indices['odf_calle'] = col_idx
                    elif 'hilo' in val_lower:
                        col_indices['hilo'] = col_idx

                # Siempre tomar la columna G (índice 6) como ANILLO ORIGEN
                col_indices['anillo_origen'] = 6
                ...
                if all(k in col_indices for k in ['equipo', 'interfaz', 'patcheo']):
                    for row_idx, row in enumerate(ws.iter_rows(min_row=2, max_row=ws.max_row, values_only=True), start=2):
                        if ws.row_dimensions[row_idx].hidden:
                            continue
                        if any(cell is not None for cell in row):
                            equipo_val = row[col_indices['equipo']]
                            interfaz_val = limpiar_puerto_completo(row[col_indices['interfaz']])
                            patcheo_val = row[col_indices['patcheo']]
                            odf_calle_val = row[col_indices.get('odf_calle')] if 'odf_calle' in col_indices else None
                            hilo_val = row[col_indices.get('hilo')] if 'hilo' in col_indices else None
                            # Tomar la columna G (índice 6) como ANILLO ORIGEN
                            anillo_origen_val = row[6] if len(row) > 6 else None
                            # Extraer el anillo usando la función
            ...
            for sn in sheet_names:
                if '-' in sn:
                    ...
                    break
            if target_sheet is not None:
                df = pd.read_excel(archivo, sheet_name=target_sheet, engine=engine, dtype=str, usecols="A:G")
                ...
                col_odf_calle = None
                col_hilo = None
                col_anillo_origen = df.columns[-1] if len(df.columns) >= 7 else None

                for col in df.columns:
                    col_lower = col.lower()
                    if 'equipo' in col_lower and col_equipo is None:
                        col_equipo = col
                    elif 'interfaz' in col_lower and 'puerto' in col_lower and col_interfaz is None:
                        col_interfaz = col
                    elif 'patcheo' in col_lower and col_patcheo is None:
                        col_patcheo = col
                    elif 'odf' in col_lower and 'calle' in col_lower and col_odf_calle is None:
                        col_odf_calle = col
                    elif 'hilo' in col_lower and col_hilo is None:
                        col_hilo = col
        ...
        print(f"Error en leer_un_archivo_is({archivo}): {e}", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)

    if not filas_archivo:
        filas_archivo.append({
            IS_COL_FIBRA: fibra,
            IS_COL_EQUIPO: None,
            IS_COL_INTERFAZ: None,
            IS_COL_PATCHEO: None,
            IS_COL_ODF_CALLE: None,
            IS_COL_HILO: None,
...
def generar_hoja_is(ruta_cables, df_op, df_ot=None):
    # Cargar datos de anillos
    anillos_dict = cargar_datos_anillos(RUTA_ANILLOS_CAMBIO)
    
    try:
        dict_busqueda, tiene_xg_por_slot, pacheo_dict, id_a_id_bay = cargar_y_preprocesar_ports(RUTA_PORTS)
    except Exception as e:
        print(f"Error al cargar ports en generar_hoja_is: {e}", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for idx, future in enumerate(as_completed(futures), 1):
            sys.stdout.write(f"\rProcesando IS: [{idx}/{total}] archivos")
            sys.stdout.flush()
            try:
                filas = future.result()
    ...
    nuevas_filas = []
    total_filas_is = len(df_final)
    for idx, (_, fila) in enumerate(df_final.iterrows(), start=1):
        if idx % 100 == 0 or idx == total_filas_is:
            sys.stdout.write(f"\rProcesando filas IS para extracción de IDs: [{idx}/{total_filas_is}]")
            sys.stdout.flush()
        ...
        if pd.isna(patcheo) or str(patcheo).strip() == '':
            sincroniza = "F (sin PATCHEO)"
        else:
            patcheo_str = str(patcheo).strip()

            if '-' not in patcheo_str:
                sincroniza = "F (formato PATCHEO sin guion)"
                op = patcheo_str
            else:
                ...
                if resto == '':
                    sincroniza = "F (PATCHEO sin bandejas)"
                else:
                    bandejas_raw = [b.strip() for b in resto.split('/') if b.strip() != '']
                    if not bandejas_raw:
                        sincroniza = "F (PATCHEO sin bandejas)"
                    else:
                        def extraer_numero_y_letra(valor):
                            valor = valor.strip()
                            match_num = re.search(r'(\d+)', valor)
                            numero = match_num.group(1) if match_num else ''
                            match_letra = re.search(r'([A-Za-z])', valor)
                            letra = match_letra.group(1) if match_letra else ''
                            return numero, letra

                        letra_comun = ''
                        for b in bandejas_raw:
                            _, letra = extraer_numero_y_letra(b)
                            if letra:
                                letra_comun = letra
                                break
                        ...
                        bandeja_raw = bandejas_raw[0]
                        num_bandeja, _ = extraer_numero_y_letra(bandeja_raw)
                        ...
                        if equipo and interfaz:
                            ...
                            if clave not in op_existe:
                                ...
                                sincroniza = "F (ID no coincide)"
                        else:
                            sincroniza = "F (equipo o interfaz vacío)"

                        bandeja = num_bandeja
                        cassetera_val = letra_comun if letra_comun else None
        ...
        if equipo == 'BOSCHU930601' and interfaz == '6/1/20':
            id_bb = buscar_id_backbone_directo(equipo, interfaz, dict_busqueda, tiene_xg_por_slot)
            print(f"DEBUG id_backbone={id_bb}")
            print(f"DEBUG id_port_pacheo={id_port_pacheo}")
            if id_bb:
                print(f"DEBUG id_bay_backbone={id_a_id_bay.get(str(id_bb))}")
            if id_port_pacheo:
                print(f"DEBUG id_bay_pacheo={id_a_id_bay.get(str(id_port_pacheo))}")
        ...
        nuevas_filas.append({
            ...
            IS_COL_OP: op,
            IS_COL_CASSETERA: cassetera_val,
            IS_COL_PATCHEO: patcheo,
            IS_COL_BANDEJA: bandeja,
